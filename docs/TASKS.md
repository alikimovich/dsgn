...
## Launch-failure diagnosis card removed (LKM-56, #197) — follow-ups

The card itself is gone (see PROGRESS). Two loose ends were left deliberately,
both confirmed as the user's call on 2026-08-12:

- [ ] **The diagnosis ENGINE is kept but has no caller.** `src/main/diagnose.ts`,
      `diag-cache.ts`, `diag-rules.ts`, the `diagnose:*` IPC + preload bridge and
      the `Diagnosis`/`DiagStep` types in `src/shared/api.ts` all survive
      unreferenced by the renderer; `test/diag-cache.mjs` and `test/diag-rules.mjs`
      still cover them. Kept on purpose — it's cheap, and re-surfacing diagnosis
      somewhere non-intrusive (through the chat, rather than a layout-displacing
      card) shouldn't have to rebuild the rules. Delete it only if that plan is
      abandoned; deleting means the types in `shared/api.ts` go too.
- [ ] **The `authNeeded` banner has the same layout bug the card had.** It sits in
      the same top-of-`.app` slot in `App.tsx` and displaces the frameless layout
      down over the traffic lights exactly as `DiagnoseCard` did. Left alone
      because it's a different state and may want a different fix (relocate
      rather than delete — the user still needs to be told auth failed).
